...
**Status:** planning document, v1 (2026-09-02, revised 2026-09-06)
...
5. **Deterministic, lockstep simulation.** CARLA runs in synchronous mode, `fixed_delta_seconds = 0.05`. One process (the world manager) owns `world.tick()` and does **not** tick again until it has received the `ControlCommand` stamped for the current tick (see `02_interfaces.md` §2). Every rate in the stack is a multiple of the 20 Hz tick; nothing runs faster than one callback per tick, and nothing is timed by wall-clock inside the stack. `realtime_factor` only slows the loop down for humans watching; it never changes results. Inside a tick, every node waits for all of its inputs stamped with the current tick before it runs (the *current-tick barrier*, `02_interfaces.md` §2), so delivery order cannot change results either. Every node publishes on every tick it acts on, even before it has a usable input (an `emergency_stop` command, a `source: "none"` stop trajectory, a pose with `valid: false`), so the gate is never starved at episode start or during localization initialization (the *no-input convention*, `02_interfaces.md` §2). No node carries a wall-clock timer of its own: a fallback path (constant-velocity prediction, lattice-only planning) engages only after `world_manager` has reported a lockstep timeout on `/nuway/sim/tick_timeout`, and such a route is always flagged. The stack's outputs are a pure function of its tick-stamped inputs, but CARLA itself is not bit-reproducible (M0 §5 task 14: two episodes of one route diverge from their first tick at 1e-6 m), so the classical/GT profiles are reproducible *up to CARLA's own spread*, which M1 measures and states; the learned profiles (M3 on) add CARLA's best-effort camera topics dropping about 1 % of frames non-deterministically and GPU inference kernels that are not bit-stable, so their reports state the score spread over repeated runs (`M1_classical_planning.md` §5).
6. **Runtime nodes in C++ (rclcpp); ML inference nodes and GT cheat twins in Python (rclpy); training in Python.** The C++ rule is about the *learned closed-loop path*, not about ground truth. A cheat twin is measurement scaffolding: it is never the thing a learned component ships behind, it is the thing the learned component is measured against, and it is written in whichever language lets its logic be **single-sourced with the offline code that generates the labels**. Point clouds and grids therefore cross into Python in exactly four nodes, all deliberate: the ML inference nodes that consume or produce them (`perception_node.py`, `prediction_node.py`), and the two cheat twins that need them (`gt_perception_node.py` reads the semantic cloud; `gt_planning_node.py` reads the occupancy grid because the M6 expert is written against `OccupancyGridMC`). No other Python node subscribes to a point cloud or a grid, and no C++ node receives one from Python except through `nuway_msgs` topics. The one twin that stays C++ is `gt_pose_node`: it publishes every tick, its publication is what triggers the controller, it must stay swap-identical with the C++ `pose_extrapolator_node` (M5), and at its size there is no duplicated implementation to remove (`M0_bringup.md` §5, 2026-09-06).
...
| M4 | Traffic light perception | Map-projected crop classifier, lane–traffic-light association verified against CARLA on every town, visibility latch near the stop line. Replaces GT traffic lights. | State accuracy > 0.97 within 40 m; red-light infractions ≤ 1.5× and driving score ≥ 95 % of the M3 learned-perception run |
| M5 | Localization | KISS-ICP-style LiDAR odometry, offline mapping of **every collection town** (the maps also feed M6's static occupancy), scan-to-map registration, fixed-lag smoother (GTSAM), TF publisher. Replaces GT pose. **Milestone: the stack drives with zero GT.** | ATE < 0.2 m; driving score drop vs GT pose < 10%; Leaderboard completions + score parity (deferred from M1) |
...
| ROS 2 | Jazzy | rmw: CycloneDDS. iceoryx shared memory is available but **off by default**: it only engages for fixed-size messages, so it does nothing for point clouds or images (`04_setup.md` §5) |
| CARLA | 0.9.16 (UE4) | Not 0.10.x (UE5: heavier GPU, native ROS 2 less stable). **Must run at default (Epic) quality — see below** |
| Python | 3.12 (system, Jazzy's) | `uv`-managed venv at repo root (`uv sync`, `uv.lock` committed, system site packages for rclpy), `torch>=2.4` on the `cu126` wheel index as the `nuway-ml[torch]` extra, pulled in by the `train` and `infer` groups; a GT-only machine syncs without it (`03_style_and_conventions.md` §6.1). CARLA 0.9.16 publishes a `cp312` client wheel, so the client matches Jazzy's interpreter |
...
| C++ | C++17, GCC 13 (Clang 18 locally), CMake ≥ 3.28 via `ament_cmake`, Ninja + ccache + mold | Eigen, GTSAM 4.2, OSQP + osqp-eigen, nanoflann, pugixml (OpenDRIVE parsing), PCL (I/O only); non-apt libs as `*_vendor` packages |
| Leaderboard | CARLA Leaderboard 2.x + scenario_runner, cloned to `external/` at pinned commits by `tools/eval/setup_leaderboard.sh` | `03_style_and_conventions.md` §6.4; their Python deps are the `leaderboard` uv group. Not vendored, not a submodule |
...
**The budgets add, they do not overlap.** The current-tick barrier serializes the chain `pose → perception → traffic lights → prediction → planner → MPC` on every planning tick, so the wall-clock cost of a planning tick is the *sum* of the rows plus the CARLA tick: on Town05 ≈ 50 + 40 + 5 + 20 + 25 + 35 + 3 ≈ 180 ms, and a control-only tick ≈ 50 + 2 + 3 ≈ 55 ms. The full learned stack therefore runs at ≈ 8–9 ticks/s, i.e. **≈ 0.4× realtime**, and that — not the CARLA-only table in §4 — is the number every wall-clock estimate for a learned profile must use (`M8_learned_planner_dagger.md` §4, `M2_perception_data_pipeline.md` §4).
...
- Time: all stamps are simulation time from `/clock`. `use_sim_time: true` everywhere. Rates are stated in ticks (1 tick = 0.05 s): "20 Hz" means every tick, "10 Hz" every *even* tick (`02_interfaces.md` §2 tick index). No node claims a rate above 20 Hz. Sim time is monotonic for the life of a stack: one stack per town, resets never reload.
...
- Routes: the harness publishes the whole route once as `/nuway/route/waypoints`; the route planner plans through all waypoints at once and republishes the reference line only on reroute (`02_interfaces.md` §3.3).
- Tests: C++ (gtest) and Python (pytest) unit tests must pass in `colcon test` / `uv run pytest` (root `testpaths`: `ml/tests`, `tools`, `tests`). Every milestone adds an integration test runnable with `tools/eval/run_routes.py --profile <milestone>`.
