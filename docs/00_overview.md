...
6. **Runtime nodes in C++ (rclcpp); ML inference nodes in Python (rclpy + torch); training in Python.** Point clouds and grids cross into Python in exactly one place: the ML inference nodes that consume or produce them (`bevfusion_node.py`, `flow_matching_node.py`). No other Python node subscribes to a point cloud or a grid, and no C++ node receives one from Python except through `nuway_msgs` topics.
