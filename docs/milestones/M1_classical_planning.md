...
**The node runs in every profile.** It always publishes `/nuway/prediction/fallback_samples`; when it is the primary producer (`prediction.source: const_vel`) it publishes `/nuway/prediction/samples` as well, with identical content. From M7 on, the consumers below fall back to `fallback_samples` when the learned node's `samples` for the current planning tick are missing (`02_interfaces.md` §3.6), which is what lets the lattice path survive a dead prediction process (M8 §2).

### 3.2 `nuway_planning/behavior_fsm` (C++)

Inputs: ego state, reference line, agents (map frame), predictions (`samples`, else `fallback_samples`, `02_interfaces.md` §3.6), traffic lights, lane graph. Runs on even ticks after all inputs stamped with that tick have arrived (`02_interfaces.md` §2 barrier).
...
Independent last check on `/nuway/planning/trajectory`, using agents and predictions (`samples`, else `fallback_samples`) directly (not the planner's internal state). Runs every tick, after the pose of that tick and — on planning ticks — after the planner's output stamped with that tick (`02_interfaces.md` §2 barrier); on control-only ticks it re-checks the last planning tick's trajectory against the current pose:
...
11. [ ] `infractions.py` (incl. the ported min-speed criterion), `driving_score.py`, `route_runner.py` (one stack per town, second non-ticking client, waypoint publishing, reset event, non-deterministic flag), `report.py`, `compare_runs.py`; route XMLs; `configs/eval/scoring_lb20.yaml`.
...
19. [ ] `tools/eval/setup_leaderboard.sh` with the Leaderboard + scenario_runner commits pinned (verify the ROS 2 track exists in that Leaderboard commit and that the pair targets CARLA 0.9.16; record both hashes in `03_style_and_conventions.md` §6.4), the `leaderboard` uv group; `leaderboard_agent.py` (watchdog brake command, shared `LongitudinalMap`, reliable sensor republish), `rig_leaderboard.json` (+ parity test vs `rig_dev.json`), `configs/profiles/leaderboard.yaml`, `run_leaderboard.sh`; run a dev route under the official runner as the mechanical integration check (route completions and score parity are M5 criteria, §3.12).
