...
1. [ ] `const_vel_node` (+ lane-follow option, unit test on a curved lane); `ros_conv.h::AgentsToMap` (the §3.1 frame rule, test against a hand-transformed agent); `prediction.launch.py` launches it in every profile.
