...
- If deserialization + pillarization exceeds **8 ms p50**, the escalation path is `lidar_preproc_node.cc` (C++, LibTorch pillarization, tensor handed over through CUDA IPC). It is not built unless the threshold is crossed; the decision is recorded here.
