...
- [ ] Leaderboard (deferred from M1, `M1_classical_planning.md` §3.12): `run_leaderboard.sh` with `m5_no_gt.yaml` (one stack per route) completes ≥ 8 of the 10 M1-protocol routes under the official runner, and the Leaderboard's own driving score is within 5 points of our harness running the same profile on the same routes.
- [ ] Initialization never starves the lockstep gate: during the ≤ 60-tick init the extrapolator publishes `/nuway/loc/pose` with `valid: false` every tick and downstream answers with its no-input outputs (`02_interfaces.md` §2), so a route start produces no `TickTimeout`.
...
- (2026-09-06) Design review: initialization publishes invalid poses rather than nothing (`EgoState.valid`, `02_interfaces.md` §2), because a silent extrapolator starved the lockstep gate for up to two minutes of wall clock per route start and flagged every route; the georeference travels in `LaneGraph` rather than as "parameters"; the mapping routes moved with the harness to `tools/eval/routes/`.
