...
#ifndef NUWAY_COMMON_TICK_HPP_
#define NUWAY_COMMON_TICK_HPP_
...
inline builtin_interfaces::msg::Time TickStamp(std::int64_t k) {
  ...
  stamp.sec = static_cast<std::int32_t>(nanos / kNanosPerSecond);
  stamp.nanosec = static_cast<std::uint32_t>(nanos % kNanosPerSecond);
  return stamp;
}
