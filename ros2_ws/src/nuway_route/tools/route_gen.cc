...
// covered, drop waypoints along the walked centerline every spacing_m, then
// ask PlanRoute to plan those waypoints from the first one and keep the
// route only when the plan is exactly the walked lane sequence. Every
// random draw comes from one std::mt19937 seeded by --seed, so the same
...
// One generated route: the walked lanes and the waypoints sampled on them.
struct Route {
...
// before min_length_m or the planner rejects it. Lanes are never repeated
// (the visited set), so a route cannot circle a block forever, though it
// may pass close to itself.
std::optional<Route> Walk(const nuway_map::LaneGraph& graph,
  ...
  // Distances are accumulated along the concatenated centerline samples.
  double along = 0.0;
...
// Loads the map, draws routes from distinct start lanes until --routes are
// found (or 2000 attempts are spent), and writes the XML. Exit codes: 0 ok,
// 1 map/route/file failure, 2 bad arguments.
int main(int argc, char** argv) {
  ...
  std::set<std::uint32_t> used_starts;  // one route per start lane
  for (int attempt = 0;
  ...
  // Leaderboard layout: <routes><route id town><waypoints><position .../>.
  // Positions go through LocationFromRos (the one place allowed to convert,
  // nuway_common/carla_conv.h), which flips the sign of y.
